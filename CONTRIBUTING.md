...
Параметр — часть, которая принадлежит функции, составляет сигнатуру функции или метода, член функции или метода.
Аргумент — значение, которое передают в функцию или метод «снаружи», чтобы определить значение параметра.
Аргумент допускается передавать в функию, передать в функцию параметр — нельзя.
