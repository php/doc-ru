...
без сопровождающих слов, которые указывают, к какому множеству относится термин,
...
Уточняющее слово разрешено добавлять один раз, если оно встречается несколько раз в предложении или абзаце, по смыслу.
Добавляя уточняющее слово, нужно стараться дать наиболее точное, или узкое, определение. Например, `$_SERVER` можно записать
как «Переменная $_SERVER», «Глобальная переменная $_SERVER» или «Суперглобальная переменная $_SERVER» — лучше предпочесть последнее.
