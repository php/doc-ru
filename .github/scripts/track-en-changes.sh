#!/usr/bin/env bash
#
# Track merged PRs in php/doc-en and create issues in doc-ru
# for files that need translation updates.
#
# How it works:
#   - Fetches PRs merged in doc-en during the last 7 days
#   - For each PR, checks if a matching issue already exists in doc-ru
#   - If not, creates one listing the RU files to update
#
...
while read -r PR; do
  ...
  # Skip if no RU-relevant files
  if [ -z "$UPDATE_LIST" ] && [ -z "$NEW_LIST" ]; then
    echo "  -> No RU-relevant files, skipping."
    continue
  fi
  ...
  if [ -n "$UPDATE_LIST" ]; then
    BODY+=$'\n'"**Файлы RU для обновления**"$'\n'
    BODY+="$UPDATE_LIST"
  fi

  if [ -n "$NEW_LIST" ]; then
    BODY+=$'\n'"**Новые файлы EN (ещё не переведены)**"$'\n'
    BODY+="$NEW_LIST"
  fi
